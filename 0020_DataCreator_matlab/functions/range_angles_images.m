function [rm_azm, rm_elm, roi_grids, torso_xyz_all, f] = range_angles_images(rm, azm, elm, jnts_xyz, rdr, show_fig)
    ...
    for t_idx = 1:Nt
        ...
        if show_fig
            ...
            xlabel('ROI range'); ylabel('ROI Angle')
            subplot(2,2,4);imagesc(roi_elm_out); title('rem-ROI');
            xlabel('ROI range'); ylabel('ROI Angle')

            pause(0.00005);
            %saveas(f,['rimg_',num2str(t_idx),'.jpg'])
        end
        '';
    end
    '';
